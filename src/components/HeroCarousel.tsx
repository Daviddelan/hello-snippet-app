...
const HeroCarousel = () => {
  ...
  const loadEvents = async () => {
    try {
      ...
      // Skip auth check for public events - not needed
      console.log('👤 HeroCarousel: Skipping auth check for public events');
      
      console.log('📊 HeroCarousel: Attempting to query events table...');
      
      // Query events directly without timeout
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          ...
          console.error('❌ HeroCarousel: Database error:', {
            code: error.code,
            message: error.message,
            details: error.details
          });
